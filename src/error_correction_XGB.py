...
def load_and_prepare_data(file_path, is_hybrid):
    ...
    print(f"Created the target variable error")

    data = data.dropna(subset=['sea_water_temperature','estimatedStaticFOC'])
    print("Dropped rows with missing sea_water_temperature.")
...
def split_and_evaluate_moving_window(data, features, target, no_est_params, train_window=8, val_window=1):
    ...
    for i in range(len(unique_months) - window_size + 1):
        ...
        val_data.loc[:, 'corrected_foc'] = val_data['estimatedStaticFOC'] + y_pred
        corrected_foc = val_data['corrected_foc'].values

        mae_corrected = mean_absolute_error(val_data['fuel_consumption'], corrected_foc)
        mse_corrected = mean_squared_error(val_data['fuel_consumption'], corrected_foc)
        rmse_corrected = np.sqrt(mse_corrected)
        r2_corrected = r2_score(val_data['fuel_consumption'], corrected_foc)
        mape_corrected = np.mean(np.abs((val_data['fuel_consumption'] - corrected_foc) / val_data['fuel_consumption'])) * 100
...
def plot_predictions_vs_actuals(speed_val, fuel_measurements, corrections, output_path):
    plt.figure(figsize=(12, 8))
    sns.set(style="darkgrid")

    sns.scatterplot(x=speed_val, y=fuel_measurements, color='orange', label='Measurements', s=50, alpha=0.4, edgecolor='k')
    sns.scatterplot(x=speed_val, y=corrections, color='blue', label='correction_model', s=50, alpha=0.4, edgecolor='none')

    plt.xlabel('Speed Over Ground', fontsize=14)
    plt.ylabel('Fuel Mass Flow', fontsize=14)
    plt.title('XGBOOST: Error Correction model with static estimates as a feature', fontsize=16)
    plt.legend(prop={'size': 12})

    plt.savefig(output_path)
    plt.close()

def plot_error_distribution(corrections, actuals, output_path):
    error_percentage = (corrections - actuals) / actuals * 100

    plt.figure(figsize=(12, 6))
    sns.histplot(error_percentage, bins=40, kde=False, color='skyblue')
    plt.yscale('log')
    plt.title('XGBOOST: Error Correction model: Prediction Errors (%) with Logarithmic Scale')
    plt.xlabel('Error (%)')
...
def plot_real_vs_predicted(test_df, corrections, output_path1, output_path2):
    sns.set(style="whitegrid")

    plt.figure(figsize=(14, 7))
    plt.plot(test_df.index, test_df['fuel_consumption'], 'g-', label='The real value', linewidth=2)
    plt.plot(test_df.index, corrections, 'r--', label='The predicted value', linewidth=2)
    plt.title('XGBOOST: Error Correction model: Fuel Consumption: Real vs Predicted', fontsize=16)
    ...
    error = (corrections - test_df['fuel_consumption']) / test_df['fuel_consumption'] * 100

    plt.figure(figsize=(14, 7))
    plt.plot(test_df.index, error, 'b-', label='Error', linewidth=1)
    plt.title('XGBOOST: Error Correction model: Prediction Error (%)', fontsize=16)
    plt.xlabel('Data Point', fontsize=14)
...
def main(file_path, tune_model, is_hybrid, use_random_split, use_moving_window):
    ...
    - is_hybrid (bool): Whether the model is hybrid.
    - use_random_split (bool): Whether to use random splitting of the dataset.
    - use_moving_window (bool): Whether to use the moving window splitting method for evaluation.

    Returns:
    None
    """
    print("Starting main workflow...")
    experiment_name = "XGBoost Hybrid Correction Model" if is_hybrid else "XGBoost Correction Model"
    ...
    with mlflow.start_run() as run:
        ...
        columns_to_drop = ['fuel_consumption','error','timestamp']
        ...
        test_df['corrected_foc'] = test_df['estimatedStaticFOC'] + y_pred

        metrics = evaluate_model(test_df['corrected_foc'], test_df['fuel_consumption'])
        ...
        # Plots 
        fuel_measurements = test_df['fuel_consumption']
        corrections = test_df['corrected_foc']

        plot_predictions_vs_actuals(X_test['speed_over_ground'], fuel_measurements, corrections, "predictions_vs_actuals.png")
        mlflow.log_artifact("predictions_vs_actuals.png")

        plot_error_distribution(corrections, fuel_measurements, "error_distribution.png")
        mlflow.log_artifact("error_distribution.png")

        plot_real_vs_predicted(test_df, corrections, "real_vs_predicted.png", "prediction_error.png")
...
#python error_correction_XGB.py --file_path '../data/steady_state_with_stamps1.csv' --tune_model False --is_hybrid True --use_random_split False --use_moving_window True